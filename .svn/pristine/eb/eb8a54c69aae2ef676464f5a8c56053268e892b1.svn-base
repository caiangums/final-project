...
public:
    ...
    int get() { return (_status == OK) ? _co2 : INVALID; }
    int sample() { update_data(); return get(); }

    int co2() { update_data(); return get(); }
    int temperature() { update_data(); return (_status == OK) ? _temperature : INVALID; }
    int humidity() { update_data(); return (_status == OK) ? _humidity : INVALID;}
