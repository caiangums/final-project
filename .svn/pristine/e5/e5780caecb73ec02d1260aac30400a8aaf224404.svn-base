...
class TSTP::MAC<Radio, true>: private SmartData, public Radio
{
private:
...
protected:
    ...
    bool pre_notify(Buffer * buf) {
        ...
        if(_in_rx_mf) { // State: RX MF (part 2/3)
            ...
            db<TSTP>(TRC) << "TSTP::MAC::pre_notify: Frame received: " << buf->frame() << " at " << Radio::Timer::count2us(buf->sfd_time_stamp) << endl;
...
private:
    ...
    static void tx_mf(const IC::Interrupt_Id & id) {
        ...
        if(_mf.dec_count() > 0) {
            ...
            _tx_pending->frame()->data<Header>()->last_hop(_mf_time + Timer::us2count(TX_DELAY + Tu));
            while(!Radio::tx_done());
...
// Class attributes
template<typename Radio>
typename TSTP::MAC<Radio, true>::Microframe TSTP::MAC<Radio, true>::_mf;

template<typename Radio>
typename TSTP::MAC<Radio, true>::Time_Stamp TSTP::MAC<Radio, true>::_mf_time;

template<typename Radio>
typename TSTP::MAC<Radio, true>::Hint TSTP::MAC<Radio, true>::_receiving_data_hint;

template<typename Radio>
TSTP::Buffer::List TSTP::MAC<Radio, true>::_tx_schedule;

template<typename Radio>
TSTP::Buffer * TSTP::MAC<Radio, true>::_tx_pending;

template<typename Radio>
bool TSTP::MAC<Radio, true>::_in_rx_mf;

template<typename Radio>
bool TSTP::MAC<Radio, true>::_in_rx_data;

template<typename Radio>
unsigned int TSTP::MAC<Radio, true>::_receiving_data_channel;

template<typename Radio>
unsigned int TSTP::MAC<Radio, true>::_silence_periods;


// TSTP::MAC without duty cycling and without microframes
template<typename Radio>
class TSTP::MAC<Radio, false>: private SmartData, public Radio
{
protected:
    typedef CPU::Reg16 Frame_ID;
    typedef CPU::Reg32 Hint;
...
private:
    ...
    static void update_tx_schedule(const IC::Interrupt_Id & id) {
        ...
        if(_tx_pending && (_tx_pending != tx_already_pending)) { // Transition: [TX pending]
            ...
            // The precise time when this frame is actually sent by the physical layer
            _tx_pending->frame()->data<Header>()->last_hop(now_ts + offset + Timer::us2count(Radio::RX_TO_TX_DELAY + CCA_TIME));
            Radio::copy_to_nic(_tx_pending->frame(), _tx_pending->size());
...
template<typename Radio>
TSTP::Buffer::List TSTP::MAC<Radio, false>::_tx_schedule;

template<typename Radio>
TSTP::Buffer * TSTP::MAC<Radio, false>::_tx_pending;

template<typename Radio>
TSTP::Statistics TSTP::MAC<Radio, false>::_stats;
